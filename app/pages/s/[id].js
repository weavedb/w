  ...
  useEffect(() => {
    if (!isNil(router.query.id)) {
      ...
      ;(async () => {
        ...
        let post = await db.cget("posts", aid)
        if (!isNil(post)) {
          setTweet(post)
          ...
          if (!isNil(post.data.body)) {
            try {
              ...
              const json = await fetch(`https://arweave.net/${url}`, {
                mode: "cors",
                }).then(v => v.json())
              */
              let body = await fetch(`https://arweave.net/${url}`, {
                mode: "cors",
              }).then(v => v.text())
              const format = "md"
      ...
      if (!isNil(user)) {
        ...
        if (ids.length > 0) {
          try {
            ...
            for (let v of ids) if (isNil(new_likes[v])) new_likes[v] = null
